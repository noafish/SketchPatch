...
class BaseOptions():
	...
	def initialize(self, parser):
		...
		parser.add_argument('--subname', type=str, default='local', help='determines scope of model - local / global')
	...
	def print_options(self, opt):
		...
		if opt.save_dir == '':
			...
			if os.path.exists(expr_dir):
				now = datetime.now()
				cmd = 'mv %s %s_%d_%d_%d_%d_%d' % (expr_dir, expr_dir, now.day, now.month, now.year, now.hour, now.minute)
				os.system(cmd)
